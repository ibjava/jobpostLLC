		...
		if (!empty($locations)){
			...
			<ul class="filter-location filter-joblist" id="location_filter">
				<?php foreach ($locations as $loc) { 
					...
					<li><a href="<?php echo home_url('?location='.trim($loc)) ?>" rel="location" data="<?php echo trim($loc) ?>" class="<?php echo $activated ? 'active' : '' ?>" ><?php echo trim($loc) ?></a></li>
				<?php } ?>
			</ul>
			<?php 
		}
...
function et_register_sidebar () {
	...
	register_sidebar ( array(
			'name' => __( 'Homepage Top Sidebar blog listing', ET_DOMAIN ),
			'id' => 'sidebar-blog-listing-home-top',
			'description' => __( 'Drop widgets here to position them at the top of your Homepage, below the demonstration text', ET_DOMAIN ),
			'before_widget' => $before_widget,
			'after_widget' => "</aside>",
			'before_title' => '<div class="widget-title">',
			...
			<div class="thumbs"><?php
				if (!empty($company_logo)){
					...
					<a id="job_author_thumb" href="<?php echo $company['post_url']?>" 
						...
						<img src="<?php echo $company_logo['company-logo'][0]; ?>" id="company_logo_thumb" data="<?php echo $company_logo['attach_id'];?>" />
					</a>
					<?php
				} ?>
			</div>
...
function JE_Company_Count ($args = '') {
	...
	?>
		<div class="first">
			<?php _e('Companies', ET_DOMAIN); ?>
			<span class="impress"><?php echo $companies_count ?></span>
		</div>
		<div>
			<?php _e('Available jobs', ET_DOMAIN); ?>
			<span class="impress"><?php echo $count['publish'] ?></span>
		</div>	
...
function ET_Widget_Job_Categories ($args = '' , $instance = array('title' => '')) {
	...
	?>	
		...
		<?php  //add more category to list 
      $jtargs = wp_parse_args( array(), array('orderby' => 'id', 'order' => 'DESC','hide_empty' => false ,'pad_counts' => true) );
	    $jtcategories = get_terms('job_type', $jtargs); 
      et_template_front_category($jtcategories, 0, array( 'title' => '', 'hide_empty' => false , 'hide_jobcount' => false)); 
      $jtargs = wp_parse_args( array(), array('orderby' => 'id', 'order' => 'ASC','hide_empty' => false ,'pad_counts' => true) );
	    $jtcategories = get_terms('job_position', $jtargs); 
      et_template_front_category($jtcategories, 0, array( 'title' => '', 'hide_empty' => false , 'hide_jobcount' => false)); 
    ?>
