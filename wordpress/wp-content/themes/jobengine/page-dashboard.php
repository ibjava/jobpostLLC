				...
				if ( $query->have_posts() ) : ?>
					...
					?>
						<li class="acc-job-item job-item-<?php echo $status ?>">
							...
							<div class="job-status apps f-right color-<?php echo $statuses[$post->post_status]['class'] ?>" data="<?php echo $post->post_status ?>">
								<?php if($i == 0) { 
							...
							<div class="title"> 
								<a href="<?php the_permalink(); ?>" title="<?php the_title() ?>"><?php the_title() ?></a> <span class="date"><?php echo get_the_date() ?></span>
								<?php 
