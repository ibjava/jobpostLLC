 ...
 */
/*
function et_job_cat_select($name, $label = 'Select Category', $args = array()){
	...
	<select name="<?php echo $name ?>" id="<?php echo $args['id'] ?>" class="<?php echo $args['class'] ?>">
		...
		<?php self::job_cat_children_options('job_category', $cats); ?>
	</select>
	<?php
}*/
...
class JE_Job_Edit extends ET_Base{
	...
	public function save_job($post_id){
		...
		// 
		$plans = et_get_payment_plans();
		if (isset($plans[$_POST['et_job_package']])){
