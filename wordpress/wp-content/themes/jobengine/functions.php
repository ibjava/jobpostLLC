...
function et_template_front_category($categories = false, $parent = 0 , $args = array()){
	...
	*/
	...
	if ( !empty($categories) ){ //print_r($categories);?>
		<ul data-tax="<?php print $categories[0]->taxonomy; ?>" class="job-filter <?php echo $parent == 0 ? 'tax-filter category-lists filter-jobcat' : '' ?> filter-joblist" style="<?php echo ($parent != 0 && !$expand) ? 'display: none' :''   ?>">
		<?php foreach ($categories as $cat) {
			if( $args['hide_empty'] && $cat->count <= 0 ) continue;
			if ( $cat->parent == $parent ) { ?>
				<li class="cat-item cat-<?php echo $cat->term_id ?> cat-<?php echo $cat->slug ?>">
					<a data="<?php echo $cat->slug ?>" href="<?php echo get_term_link($cat, $cat->taxonomy) ?>" 
