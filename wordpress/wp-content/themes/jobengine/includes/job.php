    ...
    function print_filter_list ($query_obj , $args) {
		...
		<?php 
			foreach ($types as $type) {
			 ...
			 ?>
				<li>
					<a data="<?php echo $type->slug ?>" href="<?php echo $this->get_term_link($type) ?>" class="color-<?php if(isset($colours[$type->term_id])) echo $colours[$type->term_id] ?> <?php if( $query_obj && $query_obj->term_id == $type->term_id ) echo 'active'; ?>">
		...
		if( ( current_user_can('edit_posts') && $args['author_id'] == $user_ID) || current_user_can('edit_others_posts') || !isset($args['id']) ) {
			...
			if(isset($job_data['id']) && !empty($job_data['id']) && is_numeric($job_data['id'])) {
				...
				if( current_user_can('edit_others_posts') ) {
					...
					// update job_paid if the job is free, otherwise don't change its current value
					//$job_data['job_paid']	= ($job_data['is_free']) ? 2 : et_get_post_field($job->ID,'job_paid');
			...
			if ( !is_wp_error($job_id) ){
				...
				if(isset($job_data['author_data']) && !empty($job_data['author_data'])) {
					...
					$author_data	= array(
						...
						'apply_method'					=> $job_data['apply_method'],
...
function et_refresh_job_categories($args = array()){
	...
	$order 		= get_option('et_category_order');
	if ($order) {
...
function et_job_categories_option_list ( $parent =	0, $level = 0 , $selected = '') {
	...
	if ( !empty($cats) ){
		foreach ($cats as $cat) {
			...
			echo '<option value="' . $cat->slug . '" '.$selected.'>' .$delimeter. $cat->name . '</option>';
...
function et_reject_job($job_id, $reason = '', $refund = 0){
	...
	if ( user_can( $company, 'company') ){
		$mail = array(
			...
			'message' => apply_filters( 'et_reject_mail_body', $reason, $job_id)
		);

		$mail['message']	=	et_get_mail_header(). $mail['message'].et_get_mail_footer();
		$send_result = je_reject_job_mail($mail['to'], $mail['subject'], $mail['message'], $mail['header'] );
	}
	return $result;
}
