	...
	initialize	: function(){
		...
		if( et_globals.page_template == 'page-dashboard.php' || et_globals.page_template == 'page-profile.php' || 
			...
			if( typeof this.currentUser.get('ID') == 'undefined' ) {
				this.header.modal_login.openModalAuth({ redirect_url : window.location.href });
				return ;
			}
...
JobEngine.Views.Modal_Login = JobEngine.Views.Modal_Box.extend({
	...
	initialize : function(){
		...
		this.bind('waiting', this.waiting, this);
		this.bind('endWaiting', this.endWaiting, this);
		this.loadingBtn = new JobEngine.Views.LoadingButton({el: this.$('input#submit_login')});
	...
	*/
	doRegister : function (e) {
		...
		var options		= this.options;
		view.loadingBtn = new JobEngine.Views.LoadingButton({el: this.$('input#submit_modal_register')});

		if( $('form#modal_register').valid() ) {
			// loadingBtn.loading();
			
			JobEngine.app.auth.setUserName($target.find('input#register_user_name').val());
			JobEngine.app.auth.setEmail($target.find('input#register_email').val());
			JobEngine.app.auth.setPass($target.find('input#register_pass').val());
			JobEngine.app.auth.set('role', $target.find('input[type="radio"]:checked').val());
			console.log(JobEngine.app.auth);
...
jQuery(document).ready(function($) {
	...
	// 	headertop.css({
	// 		"position" : "fixed",
	// 		"top" : headertop.offset().top + "px"
	// 	});

	// 	jbody.css({ "margin-top": jbody.offset().top +  headertop.height() + "px" });
	// }

	// if (filter.length) {
	// 	filter.css({
	// 		"position": "fixed",
	// 		"top": filter.offset().top + "px"
	// 	});

	// 	jbody.css({ "margin-top": jbody.offset().top + filter.height() + "px" });
	// }
